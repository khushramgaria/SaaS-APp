  ...
  const columns = [
    {
      ...
      cell: (row) => (
        <span
          ...
          className="font-mono text-xs font-bold text-violet-400 bg-violet-500/10 border border-violet-500/20 px-2 py-1 rounded cursor-pointer hover:bg-violet-500/20 transition-colors"
        >
          {row.taskKey || "TASK"}
        </span>
      ),
    },
    {
      ...
      cell: (row) => (
        <span
          onClick={() => onTaskClick && onTaskClick(row)}
          className="font-semibold text-white hover:text-violet-300 transition-colors cursor-pointer"
        >
          {row.title}
        </span>
      ),
