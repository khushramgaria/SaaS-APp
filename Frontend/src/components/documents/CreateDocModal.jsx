        ...
        <div className="px-6 py-4 border-b border-slate-800 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">Set document metadata before composing.</p>
            </div>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-5 flex-1 custom-scrollbar">
          ...
          <div>
            ...
            <div className="relative">
              <Folder className="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500 pointer-events-none" />
              <select
                value={projectId}
                onChange={(e) => setProjectId(e.target.value)}
                className="w-full bg-slate-950/80 border border-slate-800 text-slate-200 text-sm rounded-lg pl-10 pr-4 py-2.5 focus:outline-none focus:border-indigo-500/60 cursor-pointer"
              >
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className={`p-3.5 rounded-xl border text-left flex flex-col justify-between transition-all cursor-pointer ${
                  accessType === "workspace"
                    ...
                    : "border-slate-800 bg-slate-950/50 hover:bg-slate-800/50 text-slate-400 hover:text-slate-200"
                }`}
              >
                <div className="flex items-center justify-between w-full mb-1">
                  <Globe className="w-4 h-4 text-indigo-400" />
                  {accessType === "workspace" && <Check className="w-4 h-4 text-indigo-400" />}
                </div>
                <div>
                  <p className="font-semibold text-xs text-slate-100">Everyone in workspace</p>
                  <p className="text-[11px] text-slate-400 mt-0.5">Open workspace access</p>
                </div>
              </button>
              ...
              >
                <div className="flex items-center justify-between w-full mb-1">
                  ...
                  {accessType === "restricted" && <Check className="w-4 h-4 text-indigo-400" />}
                </div>
                <div>
                  <p className="font-semibold text-xs text-slate-100">Specific members only</p>
                  <p className="text-[11px] text-slate-400 mt-0.5">Restricted permissions</p>
                </div>
              </button>
            </div>
          </div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center space-x-2.5 min-w-0">
                        <div className="w-7 h-7 rounded-full bg-indigo-600/20 border border-indigo-500/30 flex items-center justify-center font-bold text-xs text-indigo-300 overflow-hidden shrink-0">
                          {avatar ? (
                            <img
                              src={avatar}
                              alt={name}
          ...
          <div>
            ...
            <div className="space-y-2">
              <div className="relative">
                <Tag className="w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 text-slate-500 pointer-events-none" />
                <input
                  type="text"
                  ...
                  className="w-full bg-slate-950/80 border border-slate-800 text-slate-200 text-sm rounded-lg pl-10 pr-4 py-2 focus:outline-none focus:border-indigo-500/60"
                />
              </div>
              ...
              {tags.length > 0 && (
                <div className="flex flex-wrap gap-1.5 pt-1">
                  {tags.map((t) => (
                    <span
                      key={t}
                      className="inline-flex items-center space-x-1 text-xs px-2.5 py-1 rounded-md bg-slate-800 text-slate-300 border border-slate-700/60"
                    >
                      <span>#{t}</span>
                      <button
                        type="button"
