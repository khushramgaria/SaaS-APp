...
const DocumentDetail = () => {
  ...
  if (isLoading && !currentDocument) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-6 text-slate-500 dark:text-slate-400">
        <RefreshCw className="w-8 h-8 animate-spin text-indigo-600 dark:text-indigo-500 mb-4" />
  ...
  if (error || (!isLoading && !currentDocument)) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-6">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-8 max-w-md text-center shadow-xl">
          <ShieldAlert className="w-12 h-12 text-red-500 dark:text-red-400 mx-auto mb-4" />
          <h2 className="text-xl font-bold text-slate-900 dark:text-slate-100 mb-2">Document Not Found</h2>
  ...
  if (!isExplicitlyAllowed) {
    return (
      <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex flex-col items-center justify-center p-6">
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-8 max-w-md text-center shadow-xl">
          <Lock className="w-12 h-12 text-amber-500 dark:text-amber-400 mx-auto mb-4" />
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 flex flex-col transition-colors duration-200">
        ...
        <div className="flex items-center space-x-3">
          ...
          {canEdit && (
            <button
              ...
              className="p-2 text-slate-400 hover:text-red-500 dark:hover:text-red-400 hover:bg-slate-100 dark:hover:bg-slate-900 rounded-lg transition-colors cursor-pointer"
      ...
      <main className="flex-1 w-full max-w-4xl mx-auto px-4 lg:px-8 py-8 space-y-8">
        ...
        <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/90 rounded-2xl p-6 lg:p-8 space-y-4 shadow-xl">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
            ...
            {isRestricted ? (
              <div className="inline-flex items-center space-x-1.5 text-xs text-amber-600 dark:text-amber-400 bg-amber-500/10 px-3 py-1 rounded-full border border-amber-500/20 shrink-0 self-start">
                <Lock className="w-3.5 h-3.5" />
                <span>Restricted ({allowedMembers.length} members)</span>
              </div>
            ) : (
              <div className="inline-flex items-center space-x-1.5 text-xs text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-3 py-1 rounded-full border border-emerald-500/20 shrink-0 self-start">
          ...
          {/* Metadata Row */}
          <div className="flex flex-wrap items-center gap-4 text-xs text-slate-500 dark:text-slate-400 pt-4 border-t border-slate-100 dark:border-slate-800/80">
            {/* Author */}
            <div className="flex items-center space-x-2 pr-4 border-r border-slate-200 dark:border-slate-800/80">
              <div className="w-6 h-6 rounded-full bg-indigo-600/20 dark:bg-indigo-600/30 border border-indigo-500/30 flex items-center justify-center font-semibold text-[10px] text-indigo-600 dark:text-indigo-300">
                {author.avatar ? (
                  <img
                    src={author.avatar}
                    alt={author.name}
              ...
              <span className="text-slate-900 dark:text-slate-300 font-medium">{author.name || "Unknown"}</span>
            </div>

            {/* Last Updated */}
            <div className="flex items-center space-x-1.5 pr-4 border-r border-slate-200 dark:border-slate-800/80">
            ...
            {project && (
              <div className="flex items-center space-x-1.5 pr-4 border-r border-slate-200 dark:border-slate-800/80">
                ...
                <span className="text-indigo-600 dark:text-indigo-300 font-medium">{project.name}</span>
              </div>
            )}
            ...
            {currentDocument.tags?.length > 0 && (
              <div className="flex items-center space-x-1.5 flex-wrap">
                ...
                {currentDocument.tags.map((t) => (
                  <span
                    key={t}
                    className="text-[11px] px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700/50"
                  >
                    #{t}
                  </span>
                ))}
        ...
        <div className="bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800/80 rounded-2xl p-6 lg:p-10 shadow-xl">
          {currentDocument.content ? (
            <div
              className="prose-content max-w-none text-slate-800 dark:text-slate-200 leading-relaxed"
              dangerouslySetInnerHTML={{ __html: currentDocument.content }}
            />
          ) : (
            <p className="text-slate-400 dark:text-slate-500 italic text-sm py-8 text-center">
